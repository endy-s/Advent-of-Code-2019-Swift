...
func addPathToNextPoint(movementString: String, for lastPoint: CGPoint) -> [CGPoint] {
    ...
    case "L":
        for indexes in ((Int(lastPoint.x) - movementDistance)...Int(lastPoint.x)).reversed() {
            path.append(CGPoint(x: indexes, y: Int(lastPoint.y)))
        }
    case "U":
    ...
    case "D":
        for indexes in ((Int(lastPoint.y) - movementDistance)...Int(lastPoint.y)).reversed() {
            path.append(CGPoint(x: Int(lastPoint.x), y: indexes))
        }
    default:
...
testFirstPathPoints = testFirstPathPoints.enumerated().compactMap { (index, point) -> CGPoint? in
    if index + 1 < testFirstPathPoints.count {
        if testFirstPathPoints[index + 1] == point {
            return nil
        }
    }
    return point
}
testFirstPathPoints.remove(at: 0)

secondWire.forEach { (movement) in
    guard let lastPoint = testSecondPathPoints.last else { return }
    testSecondPathPoints.append(contentsOf: addPathToNextPoint(movementString: movement, for: lastPoint))
}

testSecondPathPoints = testSecondPathPoints.enumerated().compactMap { (index, point) -> CGPoint? in
    if index + 1 < testSecondPathPoints.count {
        if testSecondPathPoints[index + 1] == point {
            return nil
        }
    }
    return point
}
testSecondPathPoints.remove(at: 0)

intersectionPoints = testFirstPathPoints.filter { (point) -> Bool in
    return testSecondPathPoints.contains(point)
    }.map { CGPointManhattanDistance(from: CGPoint(x: 0, y: 0), to: $0) }

print(intersectionPoints.min() ?? 0.0)
